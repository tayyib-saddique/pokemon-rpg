...
def build_sprites(layers: list, tile_w: int, tile_h: int,
                  base_positions: dict, map_height: int,
                  all_sprites, collision_sprites) -> None:
    """
    ...
    for layer in layers:
        ...
        for x, y, image in layer.tiles():
            ...
            if layer.name == 'Collisions':
                Generic(pos=pos, surface=pygame.Surface((tile_w, tile_h)),
            ...
            sprite.ground_y   = tile_depth(
                x, y, tile_h, layer.name,
                base_positions, fallback=map_height + 100
            )