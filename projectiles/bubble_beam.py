...
def draw_bubble(surface, cx, cy, r, irid_index=0, alpha_scale=1.0):
    ...
    if r > 5:
        ...
        pygame.draw.circle(bubble_surf, (*irid, int(150 * alpha_scale)), (bx, by), r - 1, 1)
    ...
    if r >= 3:
        ...
        hy = by - r // 3
        hr = max(1, r // 4)
        pygame.draw.circle(bubble_surf, (*B_SHINE, int(230 * alpha_scale)), (hx, hy), hr)
        if r >= 6:
            pygame.draw.circle(bubble_surf, (*B_SHINE, int(128 * alpha_scale)), (hx + 2, hy + 2), max(1, hr - 1))
...
class TrailBubble:
    ...
    def draw(self, surface, offset=(0,0)):
        draw_bubble(surface, int(self.x - offset[0]), int(self.y - offset[1]), 
                self.r, self.irid, self.life / self.max_life)
