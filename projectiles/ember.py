...
class TrailShard:
    ...
    def draw(self, surface, offset=(0, 0)):
        ...
        w = max(1, self.w * t)
        h = max(1, self.h * t)
        if   self.colour_t > 0.7: outer, inner = C_ORANGE, C_YELLOW
        elif self.colour_t > 0.4: outer, inner = C_RED,    C_ORANGE
        else:                     outer, inner = C_DARK,   C_RED
...
class BurstShard:
    def __init__(self, x, y, dir_angle, index, total):
        angle      = dir_angle + math.tau / total * index + math.radians(random.uniform(-12, 12))
    ...
    def draw(self, surface, offset=(0, 0)):
        ...
        if   t > 0.65: outer, inner = C_YELLOW, C_WHITE
        elif t > 0.35: outer, inner = C_ORANGE, C_YELLOW
        else:          outer, inner = C_RED,    C_ORANGE
...
class Ember(BaseProjectile):
    ...
    def __init__(self, origin_x, origin_y, facing):
        ...
        self.head_shards = [
            HeadShard(self._dir, op, w, h, ph)
            for op, w, h, ph in self.HEAD_SHARDS
        ]
    ...
    def update(self, dt):
        ...
        for ts in self.trail: ts.update(dt)
        for bs in self.burst: bs.update(dt)
