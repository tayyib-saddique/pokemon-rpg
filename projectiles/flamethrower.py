...
def _rotated_diamond(cx, cy, w, h, angle):
    ...
    pts = [(0, -h), (w, 0), (0, h), (-w, 0)]
    return [
        (cx + cos_a * px - sin_a * py, cy + sin_a * px + cos_a * py) for px, py in pts
    ]
...
class JetParticle:
    ...
    def draw(self, surface, offset=(0, 0)):
        ...
        if t > 0.75:
            inner, outer = C_WHITE, C_YELLOW
        elif t > 0.50:
            inner, outer = C_YELLOW, C_ORANGE
        elif t > 0.25:
            inner, outer = C_ORANGE, C_RED
        else:
            inner, outer = C_RED, C_DARK
        ...
        pts_inner = _rotated_diamond(
            sx, sy, max(1, int(w * 0.55)), max(1, int(h * 0.50)), self.angle
        )
...
class NozzleFlash:
    ...
    def draw(self, surface, offset=(0, 0)):
        ...
        if r > 1:
            ...
            if r > 5:
                pygame.draw.circle(
                    surface, _blend(C_YELLOW, t * 0.70), (sx, sy), r - 4, 1
                )
